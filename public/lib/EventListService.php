...
require_once __DIR__ . '/../api/events/_functions.php';

class EventListService
{
    /**
     * Lädt alle Events (inkl. Serien-Vorkommen) innerhalb eines Zeitraums.
     *
     * @param int[] $categoryIds
     * @return array<int,array<string,mixed>>
     */
    public static function fetchEvents(PDO $pdo, DateTimeImmutable $from, DateTimeImmutable $to, array $categoryIds = [], bool $includeSystem = false, string $search = ''): array
    {
        $params = [
        ...
        foreach ($seriesRows as $seriesRow) {
            ...
            try {
                $occList = Series::generateOccurrences($seriesData, $from, $to, $seriesOverrides, $holidayIndex);
            ...
            foreach ($occList as $occ) {
                ...
                if ($search !== '' && !self::matchesSearch($occ, $search)) {
                    continue;
                }
        ...
        return array_values(array_filter($allEvents, static function (array $row) use ($search): bool {
            return self::matchesSearch($row, $search);
        }));
    }
